...
def get_mask_rm(obs_mask, ratio):
    rand_for_mask = torch.rand_like(obs_mask.float()) * obs_mask
    ...
    cond_mask = (rand_for_mask < 0).reshape(obs_mask.shape).bool()
    return cond_mask
...
class TS_Dataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ts = self.data[idx].cuda()
        obs_mask = self.obs_mask[idx].float().cuda()
        if self.dataset_mode in ["Train", "Eval"]:
            cond_mask = self.get_mask(self.obs_mask[idx]).float().cuda()
...
def sampling(net, diffusion_hyperparams, cond, mask, only_generate_missing=0):
    """
